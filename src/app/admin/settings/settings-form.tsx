...
/** Folds platform_settings rows onto the defaults. */
export function applySettingRows(rows: { key: string; value: unknown }[]): PlatformSettings {
  const loaded: Partial<PlatformSettings> = { ...defaultSettings }
  const record = loaded as Record<keyof PlatformSettings, SettingValue>
  rows.forEach((setting) => {
    if (setting.key in loaded) {
      const key = setting.key as keyof PlatformSettings
      record[key] = parseSettingValue(key, setting.value as never)
    }
  })
  return loaded as PlatformSettings
}

export function AdminSettingsForm({
  initialRows,
}: {
  initialRows: { key: string; value: unknown }[]
}) {
  // Folded here rather than on the server: applySettingRows lives in this
  // 'use client' module, and a client function cannot be called from a server
  // component - only rendered as one.
  const [settings, setSettings] = useState<PlatformSettings>(() => applySettingRows(initialRows))
  ...
  const fetchSettings = useCallback(async () => {
    ...
    try {
      ...
      setSettings(applySettingRows(payload.settings || []))
    } catch (error) {
