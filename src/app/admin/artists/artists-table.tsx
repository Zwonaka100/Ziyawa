...
import type { ArtistAdminRow } from '@/lib/admin/artists'
...
export function AdminArtistsTable({ initialArtists }: { initialArtists: ArtistAdminRow[] }) {
  // Seeded from the server render, so there is no empty first paint and no
  // fetch on mount. Refetching happens only when a filter actually changes.
  const [artists, setArtists] = useState<ArtistAdminRow[]>(initialArtists)
