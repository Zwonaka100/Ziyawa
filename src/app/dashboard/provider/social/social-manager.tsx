...
import { ProviderSocialLink, SocialPlatform, SOCIAL_PLATFORM_LABELS } from '@/types/database';

interface ProviderSocialManagerProps {
  providerId: string;
  initialLinks: ProviderSocialLink[];
}
...
const platformIcons: Record<SocialPlatform, React.ReactNode> = {
  ...
  spotify: <Globe className="h-5 w-5" />,
  apple_music: <Globe className="h-5 w-5" />,
  soundcloud: <Globe className="h-5 w-5" />,
  bandcamp: <Globe className="h-5 w-5" />,
  deezer: <Globe className="h-5 w-5" />,
...
export function ProviderSocialManager({ providerId, initialLinks }: ProviderSocialManagerProps) {
  const [links, setLinks] = useState<ProviderSocialLink[]>(initialLinks);
  ...
  // Filter to relevant platforms for providers
  const relevantPlatforms: SocialPlatform[] = [
    'website', 'instagram', 'facebook', 'youtube', 'tiktok', 'twitter', 'linkedin', 'other'
  ];
    ...
    <div className="space-y-6">
      ...
      {links.length > 0 ? (
        ...
        <div className="text-center py-16 bg-white rounded-xl border border-neutral-200">
          ...
          <p className="text-neutral-500 mb-4">Connect your business profiles to build credibility</p>
      ...
      <div className="p-4 bg-neutral-100 rounded-xl">
        ...
        <ul className="text-sm text-neutral-600 space-y-1">
          <li>• Add your business website to appear more professional</li>
          <li>• Instagram and Facebook help organizers see your past work</li>
          <li>• LinkedIn adds credibility for B2B services</li>
        </ul>
      </div>
    </div>
  );
